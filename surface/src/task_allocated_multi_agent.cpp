...
#include "surface/task_checker_node.hpp"  // <-- your header‐only class
...
class MultiSubscriberNode : public rclcpp::Node,public std::enable_shared_from_this<MultiSubscriberNode>{
    public:
        MultiSubscriberNode()
              ...
              null_matrix_{0}
        {
            ...
            // Declare the robot_name parameter with a default value
            this->declare_parameter<std::string>("robot_name", "my_lrauv_1");
            // Retrieve the parameter
            this->get_parameter("robot_name", robot_name_);

            std::string topic_1 = "/" + robot_name_ + "/submarine/odometry";
            std::string topic_2 = "/" + robot_name_ + "/submarine/Laser_scan_front";
            std::string topic_3 = "/" + robot_name_ + "/propeller/thrust";
            std::string topic_4 = "/" + robot_name_ + "/submarine/horizontal/fin/pos";
            ...
            sub_task_details_=this->create_subscription<message_type::msg::List>("/Task_Details",10,std::bind(&MultiSubscriberNode::decision_callback, this, std::placeholders::_1));


    
            // Initialize Publishers
            pub_submarine_thrust_ = this->create_publisher<std_msgs::msg::Float64>(topic_3, 10);
            pub_submarine_fin_ = this->create_publisher<std_msgs::msg::Float64>(topic_4, 10);
       ...
       void init(){
            task_checker_ = std::make_shared<TaskCheckerNode>("agent_name", this->rclcpp::Node::shared_from_this());}
           
    
    private:
    std::string robot_name_;

    std::shared_ptr<TaskCheckerNode> task_checker_;
...
//What about inverse directional space mapping
        ...
        void decision_callback(const message_type::msg::List::SharedPtr msg){
            rclcpp::Time now = this->now();
            double dt = (now - last_time_).seconds();
            last_time_ = now;
        ...
        void submarine_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
            ...
            if (dt < 1e-6) return;  // Prevent zero division
            if (task_checker_->isTaskReady()) {
                auto goal = task_checker_->getDesiredPosition();
                // Now use goal.x, goal.y as your desired_submarine_x_, desired_submarine_y_
                desired_submarine_x_ = goal.x;
                desired_submarine_y_ = goal.y;
              }
       ...
       std::vector <double> rotated= rotateConvergence(direction_space_per_obstacle(convergence_vector,global_velocity ),direction_space_per_obstacle(convergence_vector,pseudo_tangent),
